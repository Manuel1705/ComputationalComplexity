...
\triangleright, \sqcup^{j_{k-1}}, \triangleright, \sqcap^{f(|x|)}),
\]
...
$\sqcap^{f(|x|)}$, where $\sqcap$ is a ``quasi-blank'' symbol. And, on any input $x$, $M_f$ halts after 
